...
class UserModel(StreamlinedUser["OnlyFansAuthModel", "OnlyFansAPI"]):
    ...
    async def send_message(
        ...
        Returns:
            dict: The message object if successful, None if failed
        """
        import logging
        logger = logging.getLogger(__name__)
        
        endpoint = f"https://onlyfans.com/api2/v2/chats/{self.id}/messages"
        ...
        logger.info(f"Attempting to send message to user {self.id}")
        ...
        try:
            ...
            if response and isinstance(response, dict):
                if "error" in response:
                    ...
                    # Create a MessageModel from the response
                    from ultima_scraper_api.apis.onlyfans.classes import message_model
                    return message_model.MessageModel(response, self)
                else:
            ...
            logger.error(f"Exception sending message: {type(e).__name__}: {str(e)}")
